PROJECT_NAME := Okta Package
include build/common.mk

PACK             := okta
PACKDIR          := sdk
PROJECT          := github.com/pulumi/pulumi-okta
NODE_MODULE_NAME := @pulumi/okta
...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-okta/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-okta/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${TFGEN}
